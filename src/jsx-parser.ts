...
export class JSXParser extends Parser {
    ...
    parsePrimaryExpression(): Expression | JSXElement {
    ...
    parseJSXIdentifier(): JSXIdentifier {
        ...
        return this.finalize(node, new JSXIdentifier(as_string(token.value)));
    }
    ...
    parseJSXExpressionContainer(): JSXExpressionContainer {
        ...
        let expression: Expression | JSXEmptyExpression;
        if (this.matchJSX('}')) {
    ...
    parseComplexJSXElement(el: MetaJSXElement): MetaJSXElement {
        ...
        while (!this.scanner.eof()) {
            ...
            if (element.type === Syntax.JSXOpeningElement) {
