...
import { IfStatement, Program, VariableDeclaration } from './nodes';
...
export function getLoopProtectorBlocks(varName: string, timeout: number): { before: VariableDeclaration; inside: IfStatement } {
    const ast1: Program = parse(`var ${varName} = Date.now()`);
    const ast2: Program = parse(`if (Date.now() - ${varName} > ${timeout}) {throw new Error("Infinite loop suspected after ${timeout} milliseconds.")}`);
